...
export function useAccount({ onConnect, onDisconnect }: UseAccountConfig = {}) {
  ...
  const [previousAccountRef, setPreviousAccountRef] =
    createSignal<typeof account>()
  const previousAccount = previousAccountRef() ?? ({} as typeof account)
